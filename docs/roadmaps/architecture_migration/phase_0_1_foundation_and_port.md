...
## Phase 0 — Technical Foundation - dev_1.0.1
...
New `src/` directory alongside existing `arena-drifter/`. They should both be playable
...
Port `audio.js` nearly verbatim. Web Audio oscillators stay (procedural engine/drift/music synthesis). Rename `Audio` -> `AudioManager` to avoid shadowing the browser's `Audio` constructor.
...
## Phase 1 — Core Gameplay Port - dev_1.1.0
...
**Issue #7 — Mobile Controls (bug fix required during this phase):**

The existing vanilla JS touch controls have a known bug. Because Phase 1 rebuilds `inputManager.ts` from scratch, this is the right time to fix it — don't port the bug. Verification: test touch input on a real mobile device (not just browser DevTools) before marking Phase 1 complete.

**Issue #9 — Controller Support (stub here, complete in Phase 6):**

Add `InputAction` enum with a gamepad polling stub in `inputManager.ts`. Full implementation (analog steering, deadzone, rumble, rebindable controls) is deferred to Phase 6.
